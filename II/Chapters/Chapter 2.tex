...
\begin{remark}
    ...
    defines an isomorphism between the $K$-module of all quadratic forms and the $K$-module of all bilinear symmetric forms on $V$. The bilinear form $\varphi_q$ is called the \textbf{polarization} of $q$. The kernel of $\varphi_q$ is defined as
    $$\{x\in V\mid  \forall y\in V, \varphi_q(x,y) = 0\}$$
    is also called the kernel of $q$, and denoted by $\ker q$. We have $\ker q\subseteq C(q)$.
\end{remark}
\begin{definitionenv}
...
\end{proofenv}
\begin{propositionenv}
    Let $K$ be a field, $2$ is invertible in $K$, $V$ is a finite-dimensional vector space over $K$, and $\varphi : V\times V\rightarrow K$ is symmetric bilinear. There exists a basis of $V$ which is $\varphi$-orthogonal.
\end{propositionenv}
\begin{proofenv}
...
\begin{remark}
    ...
    $\tilde{q}:K^{r}\rightarrow K$ is a quadratic form with expression independent of $x_i$.
...
\begin{lemmaenv}\label{2.4.3}
    ...
    Then the kernel of $q$ is equal to the isotropic cone of $q$.
\end{lemmaenv}
\begin{proofenv}
...
\begin{propositionenv}
    ...
    which sends the class of $x$ in $E/(E\cap F)$ to the class of $x$ in $(E+F)/F$ is an isomorphism.
